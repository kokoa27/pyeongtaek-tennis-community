...
app.post('/api/posts', (req, res) => {
  ...
  const newPost = { id, title, content, author, createdAt: new Date().toISOString() };
  posts.push(newPost);
  res.status(201).json(newPost);
});

// API 엔드포인트: 날씨 정보 (실제 API 사용)
app.get('/api/weather', async (req, res) => {
  ...
  try {
    ...
    // API 호출
    const response = await axios.get(`http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst?serviceKey=${apiKey}&numOfRows=1000&pageNo=1&base_date=${baseDate}&base_time=${baseTime}&nx=${nx}&ny=${ny}&dataType=JSON`);
